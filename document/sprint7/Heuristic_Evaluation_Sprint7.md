...
| ------------------------------------ | -------- | ---------------- |
| H1 — Visibility of System Status     | 4        | 3                |
...
Following individual evaluation, the team aggregated all findings and held a debriefing discussion.

### Key Findings Agreed Upon

- **H1 (Visibility of System Status)** was the most frequently violated heuristic, with 4 issues across all evaluators. The system rarely confirms that user actions (login, save, book) were successful.
- **H3 (User Control and Freedom)** issues are the most impactful functionally: users cannot cancel bookings or edit/delete time slots, both of which are core expected features.
- **H9 (Error Recovery)** issues affect trust: error messages are generic and do not guide users toward a solution.
- Severity-3 issues in H1, H3, and H9 should be treated as high-priority backlog items for a future sprint.

### Issues Addressed During Sprint 7

Two bugs identified through manual testing and UAT (related to H1 — Visibility of System Status) were resolved during Sprint 7:

- **BUG-007** — BookingViewController now shows an `Alert` dialog after a learner successfully books a lesson, providing the missing system status feedback (H1)
...
The remaining Severity-3 issues (cancellation, time slot editing, Arabic translations) are out of scope for Sprint 7 and are documented as known gaps. The team agreed these should be addressed before final delivery if time permits.
